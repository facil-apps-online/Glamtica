...
BEGIN
  ...
  WITH user_branches AS (
    -- 1. Find all branches for the user using get_tenant_users RPC
    SELECT DISTINCT gtu.branch_id
    FROM get_tenant_users(tenant_id_param) gtu
    WHERE gtu.user_id = user_id_param AND gtu.status = 'active' -- Removed gtu.tenant_id filter
  ),
  ...
  commission_matrix AS (
    ...
    WHERE EXISTS (
      ...
      WHERE bs.service_id = rs.service_id AND bs.branch_id = ub.branch_id
    )
  )
...
BEGIN
  ...
  commission_matrix AS (
    ...
    WHERE EXISTS (
      ...
      WHERE bp.product_id = rp.product_id AND bp.branch_id = ub.branch_id
    )
  )
  -- 4. Join with commissions and aggregate
  SELECT
