...
BEGIN
    ...
    IF user_record.id IS NULL OR user_record.password_hash IS NULL OR extensions.crypt(p_password, user_record.password_hash) <> user_record.password_hash THEN
    ...
    -- Get JWT secret from Supabase's built-in setting (for passing to Edge Function)
