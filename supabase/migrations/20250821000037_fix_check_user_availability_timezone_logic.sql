-- Migration: 20250821000037_fix_check_user_availability_timezone_logic.sql
-- Description: Fixes the timezone logic in check_user_availability by removing SELECT ... INTO from subquery.
    ...
    WHERE
        ...
        AND EXISTS (
            ...
            WHERE us.user_id = tu.user_id
              ...
              -- Usar la zona horaria del usuario directamente de la unión
              AND v_appointment_start_utc::TIME >= (us.start_time::TEXT || ':00')::TIME AT TIME ZONE COALESCE(au.raw_user_meta_data->>'timezone', v_tenant_timezone) AT TIME ZONE 'UTC'::TIME
              AND v_appointment_end_utc::TIME <= (us.end_time::TEXT || ':00')::TIME AT TIME ZONE COALESCE(au.raw_user_meta_data->>'timezone', v_tenant_timezone) AT TIME ZONE 'UTC'::TIME
        )
        AND NOT EXISTS (
            SELECT 1
