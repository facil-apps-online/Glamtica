-- Migration to reload update_attention_items to fix schema cache issues
...
BEGIN
  -- Force schema reload with a trivial change (comment)
  ...
  IF array_length(p_combo_ids_to_delete, 1) > 0 THEN
    -- Also delete the services and products linked to the combo being deleted
  ...
  LOOP
    ...
    -- If the combo is NEW (id was not provided), insert its items.
