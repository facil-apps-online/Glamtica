-- Migración para corregir la función login_user y manejar el caso del super_admin
...
DECLARE
    user_record RECORD;
BEGIN
    -- Find the user and include tenant name, handling the super_admin case with COALESCE
    SELECT
        u.id,
        u.password_hash,
        COALESCE(u.tenant_id, '00000000-0000-0000-0000-000000000000') AS tenant_id, -- Default tenant_id for super_admin
        u.branch_id,
        r.name AS role_name,
        u.first_name,
        u.last_name,
        u.avatar_url,
        COALESCE(t.name, 'Glamtica Control Panel') AS tenant_name -- Default tenant_name for super_admin
    INTO
        user_record
    FROM
        public.users u
    ...
    -- Return user data including the coalesced tenant_name
