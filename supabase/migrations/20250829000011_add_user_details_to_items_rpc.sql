-- Migration to add user details to services and combos in the get_attentions_with_details RPC
...
BEGIN
  ...
  SELECT
    ...
    (SELECT json_agg(json_build_object(
        ...
        'users', (SELECT json_build_object('id', u.id, 'first_name', u.first_name, 'last_name', u.last_name) FROM public.users u WHERE u.id = aserv.user_id LIMIT 1)
    )) 
    ...
    (SELECT json_agg(json_build_object(
        ...
        'users', (SELECT json_build_object('id', u.id, 'first_name', u.first_name, 'last_name', u.last_name) FROM public.users u WHERE u.id = ap.user_id LIMIT 1)
    )) 
    ...
    (SELECT json_agg(json_build_object(
        ...
        'users', (SELECT json_build_object('id', u.id, 'first_name', u.first_name, 'last_name', u.last_name) FROM public.users u WHERE u.id = ac.user_id LIMIT 1)
    )) 
