...
-- MIGRATION: Add timezone to get_tenant_users RPC function

-- Step 1: Drop the old function.
DROP FUNCTION IF EXISTS public.get_tenant_users(uuid);

-- Step 2: Create the function with the new timezone field.
...
BEGIN
    ...
    SELECT
        ...
        (u.raw_user_meta_data ->> 'timezone')::text AS timezone -- Added timezone
    FROM
