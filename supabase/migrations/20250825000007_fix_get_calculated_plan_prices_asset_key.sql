-- MIGRATION: Enhance get_calculated_plan_prices with platform filtering and promotional prices (FINAL FIX)
...
BEGIN
    ...
    WITH
    branch_asset AS (
        SELECT id FROM public.plan_assets WHERE asset_key = 'suc_glam' LIMIT 1 -- CHANGED HERE
    ),
    current_tariffs AS (
    ...
    SELECT
        ...
        CASE -- Si la moneda base es la misma que la del país, no se convierte.
        ...
        CASE -- Lógica aplicada también al precio de la sucursal extra
        ...
        CASE -- Lógica aplicada también al precio promocional
...
COMMENT ON FUNCTION public.get_calculated_plan_prices(UUID) IS 'Rebuilt to use the versioned tariff system. It now filters by platform, excludes trial plans, calculates promotional prices, and correctly handles same-currency scenarios without conversion.';
