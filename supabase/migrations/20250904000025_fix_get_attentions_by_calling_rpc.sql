-- Migration: Corrects get_attentions_with_details to securely fetch user data by calling the get_tenant_users RPC and joining to the results.
...
BEGIN
  ...
  SELECT
    ...
    (SELECT json_agg(json_build_object(
        ...
        'users', (SELECT json_build_object('first_name', tu.first_name, 'last_name', tu.last_name) FROM tenant_users tu WHERE tu.user_id = aserv.user_id)
    )) 
    ...
    (SELECT json_agg(json_build_object(
        ...
        'users', (SELECT json_build_object('first_name', tu.first_name, 'last_name', tu.last_name) FROM tenant_users tu WHERE tu.user_id = ap.user_id)
    )) 
