...
-- MIGRATION: Refactor check_user_availability to use timezone from get_tenant_users
...
BEGIN
    -- Logs are kept for final debugging confirmation
    RAISE NOTICE '[check_availability_DEBUG_V2] START';
    RAISE NOTICE '[check_availability_DEBUG_V2] Params: item_id=%, item_type=%, branch_id=%, tenant_id=%, app_date=%, app_time=%, duration=%', p_item_id, p_item_type, p_branch_id, p_tenant_id, p_appointment_date, p_appointment_time, p_duration_minutes;

    v_day_of_week := EXTRACT(DOW FROM p_appointment_date);
    RAISE NOTICE '[check_availability_DEBUG_V2] Day of week: %', v_day_of_week;

    SELECT default_timezone INTO v_tenant_timezone FROM public.tenants WHERE id = p_tenant_id;
    RAISE NOTICE '[check_availability_DEBUG_V2] Tenant timezone from DB: %', v_tenant_timezone;

    IF v_tenant_timezone IS NULL THEN
        v_tenant_timezone := 'UTC';
        RAISE NOTICE '[check_availability_DEBUG_V2] Tenant timezone is NULL, falling back to UTC.';
    END IF;
    ...
    RAISE NOTICE '[check_availability_DEBUG_V2] Appointment Start UTC: %', v_appointment_start_utc;
    ...
    RAISE NOTICE '[check_availability_DEBUG_V2] Appointment Range: %', v_appointment_range;
    ...
    FROM
        public.get_tenant_users(p_tenant_id) tu -- This now returns the user's timezone
    LEFT JOIN LATERAL (
    ...
    -- REMOVED: LEFT JOIN auth.users au ON au.id = tu.user_id
    WHERE
            ...
            ( 
                EXISTS (
                    ...
                    WHERE us.user_id = tu.user_id
                      ...
                      AND TSRANGE(
                            -- Use tu.timezone instead of joining to auth.users
                            ( (p_appointment_date::TEXT || ' ' || us.start_time::TEXT)::TIMESTAMP AT TIME ZONE COALESCE(tu.timezone, v_tenant_timezone) AT TIME ZONE 'UTC' )::TIMESTAMP WITHOUT TIME ZONE,
                            ( (p_appointment_date::TEXT || ' ' || us.end_time::TEXT)::TIMESTAMP AT TIME ZONE COALESCE(tu.timezone, v_tenant_timezone) AT TIME ZONE 'UTC' )::TIMESTAMP WITHOUT TIME ZONE
                          ) @> v_appointment_range
                )
                AND NOT EXISTS (
                    SELECT 1
    ...
    RAISE NOTICE '[check_availability_DEBUG_V2] Found % available users matching all criteria.', v_user_count;
    RAISE NOTICE '[check_availability_DEBUG_V2] END';
