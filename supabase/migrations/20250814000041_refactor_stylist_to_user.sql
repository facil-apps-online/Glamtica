...
-- Step 1: Alter attentions table
-- Add tenant_id and branch_id columns
ALTER TABLE public.attentions ADD COLUMN tenant_id UUID;
ALTER TABLE public.attentions ADD COLUMN branch_id UUID;

-- Step 2: Alter attention_services table
-- Drop the foreign key constraint
ALTER TABLE public.attention_services DROP CONSTRAINT attention_services_stylist_id_fkey;

-- Rename the column
ALTER TABLE public.attention_services RENAME COLUMN stylist_id TO user_id;

-- Add the new foreign key constraint
ALTER TABLE public.attention_services ADD CONSTRAINT attention_services_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);

-- Step 3: Alter service_evidence table
-- Drop the foreign key constraint
ALTER TABLE public.service_evidence DROP CONSTRAINT service_evidence_uploaded_by_fkey;
...
-- Step 4: Alter attention_products table
-- Add user_id column
ALTER TABLE public.attention_products ADD COLUMN user_id UUID;

-- Add the new foreign key constraint
ALTER TABLE public.attention_products ADD CONSTRAINT attention_products_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);

-- Step 5: Update the get_dashboard_stats function
