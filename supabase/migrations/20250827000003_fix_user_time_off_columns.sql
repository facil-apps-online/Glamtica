-- Migration: Fix user_time_off column names in check_user_availability function

-- Step 1: Drop the existing function that has the incorrect column reference.
DROP FUNCTION IF EXISTS public.check_user_availability(UUID, TEXT, UUID, UUID, TIMESTAMPTZ, INTEGER, UUID, UUID);

-- Step 2: Recreate the check_user_availability function with the correct logic for user_time_off
...
BEGIN
    ...
    v_appointment_range := TSRANGE(p_appointment_datetime::TIMESTAMP, v_appointment_end_utc::TIMESTAMP, '[)');
    ...
    WHERE
        ...
        AND (
            ( -- Availability logic
                ...
                -- Check for time off (CORRECTED LOGIC)
                AND NOT EXISTS (
                    SELECT 1
                    ...
                    WHERE uto.user_id = tu.user_id
                      ...
                      -- Correctly check for overlap between the appointment and the time off period
                      AND v_appointment_range && TSRANGE(uto.start_date::TIMESTAMP, uto.end_date::TIMESTAMP, '[)')
                )
            )
