-- Migration: 20250725000032_fix_user_creation_with_direct_insert.sql
-- Description: This migration provides the definitive fix for the setup function.
-- It replaces the incorrect user invitation function call with a direct INSERT into auth.users,
-- which is the correct, documented method for creating a user with an invitation from the backend.

-- Step 1: Drop the previous version of the function.
DROP FUNCTION IF EXISTS public.create_application_superadmin(TEXT, TEXT);

-- Step 2: Recreate the function with the correct user creation logic.
...
BEGIN
    ...
    -- Create the user via direct INSERT, which is the correct backend method.
    -- This automatically sends an invitation email.
    INSERT INTO auth.users (email, raw_app_meta_data, instance_id)
    VALUES (p_admin_email, v_app_metadata, '00000000-0000-0000-0000-000000000000');
...
-- COMMENT: Corrects the user creation call to use a direct INSERT into auth.users.
