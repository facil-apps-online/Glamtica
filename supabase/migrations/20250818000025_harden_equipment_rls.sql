-- Harden RLS for equipment module

-- 1. Add tenant_id to equipment_assignments and equipment_maintenance_history
ALTER TABLE public.equipment_assignments ADD COLUMN tenant_id uuid REFERENCES tenants(id);
ALTER TABLE public.equipment_maintenance_history ADD COLUMN tenant_id uuid REFERENCES tenants(id);

-- Backfill tenant_id for existing records
UPDATE public.equipment_assignments SET tenant_id = (SELECT tenant_id FROM equipment WHERE id = equipment_id);
UPDATE public.equipment_maintenance_history SET tenant_id = (SELECT tenant_id FROM equipment WHERE id = equipment_id);

-- Alter columns to be NOT NULL
ALTER TABLE public.equipment_assignments ALTER COLUMN tenant_id SET NOT NULL;
ALTER TABLE public.equipment_maintenance_history ALTER COLUMN tenant_id SET NOT NULL;

-- 2. Re-create RPC functions with tenant_id checks

-- Function to get equipment (already has tenant_id)
-- No changes needed for get_equipment, but we include it for completeness
...
-- Function to create equipment (already has tenant_id in json)
-- No changes needed for create_equipment
...
CREATE OR REPLACE FUNCTION update_equipment(p_tenant_id uuid, p_equipment_id uuid, p_equipment_data jsonb)
RETURNS void AS $$
BEGIN
    UPDATE equipment
    SET
...
CREATE OR REPLACE FUNCTION delete_equipment(p_tenant_id uuid, p_equipment_id uuid)
RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION get_equipment_assignments(p_tenant_id uuid, p_equipment_id uuid)
RETURNS TABLE(
    id uuid,
    user_name text,
    branch_name text,
...
CREATE OR REPLACE FUNCTION assign_equipment_to_user(p_tenant_id uuid, p_equipment_id uuid, p_user_id uuid, p_branch_id uuid, p_assignment_date date)
RETURNS uuid AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION return_equipment(p_tenant_id uuid, p_assignment_id uuid, p_return_date date)
RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION get_equipment_maintenance_history(p_tenant_id uuid, p_equipment_id uuid)
RETURNS TABLE(
    id uuid,
...
CREATE OR REPLACE FUNCTION create_equipment_maintenance_record(p_tenant_id uuid, p_maintenance_data jsonb)
RETURNS uuid AS $$
DECLARE
    new_maintenance_id uuid;
BEGIN
    INSERT INTO equipment_maintenance_history (tenant_id, equipment_id, maintenance_date, notes)
    VALUES (
        p_tenant_id,
