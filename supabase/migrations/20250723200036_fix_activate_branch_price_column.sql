-- MIGRATION: Correct column name for branch price in activate_branch
...
BEGIN
    ...
    -- Step 4: Get plan details using the correct column name for the plan
    SELECT * INTO v_plan FROM public.subscription_plans WHERE id = v_subscription.subscription_plan_id;
    
    -- Step 5: CORRECTED check for the branch price column name
    IF v_plan IS NULL OR v_plan.price_per_branch IS NULL OR v_plan.price_per_branch <= 0 THEN 
        RAISE EXCEPTION 'Invalid plan or price_per_branch is not set'; 
    END IF;
    ...
    ELSE
        ...
        IF v_days_remaining > 0 AND v_days_in_cycle > 0 THEN
            v_daily_rate := v_plan.price_per_branch / v_days_in_cycle;
    ...
    VALUES (v_subscription.id, 'branch', p_branch_id, 'active', v_plan.price_per_branch);
