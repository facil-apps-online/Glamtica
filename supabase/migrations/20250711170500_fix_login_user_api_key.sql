...
BEGIN
    ...
    supabase_anon_key := current_setting('app.supabase_anon_key', true); -- Assuming you set this in your session context or env
    IF supabase_anon_key IS NULL OR supabase_anon_key = '' THEN
        RAISE EXCEPTION 'Supabase Anon Key is not configured.';
    END IF;
    ...
    payload_to_edge_function := json_build_object(
        ...
        'audience', 'https://vtfsbogpkrcbfuhhoepf.supabase.co/auth/v1' -- HARDCODED FOR DEBUGGING
    )::text;
    ...
    SELECT * FROM http_post(
        ...
        'application/json',
        ARRAY[http_header('apikey', supabase_anon_key)] -- Add API key header
