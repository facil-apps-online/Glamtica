    ...
    attention_durations AS (
        WITH service_details AS (
            SELECT 
                ...
                s.service_order,
                s.id as attention_service_id -- Explicitly alias the column
        ...
        timeline AS (
            SELECT
                attention_id,
                LAG(
                    SUM(CASE WHEN NOT is_parallel THEN duration_minutes ELSE 0 END) OVER (PARTITION BY attention_id ORDER BY service_order, attention_service_id), -- Use the alias
                    1, 0
                ) OVER (PARTITION BY attention_id ORDER BY service_order, attention_service_id) -- Use the alias
