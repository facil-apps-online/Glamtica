...
BEGIN
    -- 1. Obtener el ID del plan de suscripción desde el ID del precio del tariff
    ...
    FROM public.get_calculated_plan_prices( (SELECT platform_id FROM public.subscription_plans WHERE id = v_plan_id LIMIT 1) ) gcp
    ...
    -- 5. Calcular la nueva fecha de expiración y la nueva fecha de inicio
    ...
    -- 6. Insertar una nueva suscripción para el tenant
...
COMMENT ON FUNCTION public.activate_subscription IS 'Activa o renueva la suscripción de un tenant, actualizando su estado y fecha de vencimiento, usando la lógica de precios calculados y tarifas.';
