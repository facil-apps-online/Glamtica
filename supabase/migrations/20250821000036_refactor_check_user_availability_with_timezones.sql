-- Migration: 20250821000036_refactor_check_user_availability_with_timezones.sql
-- Description: Refactors the check_user_availability function to correctly handle timezones for schedules and time-off.
    ...
    WHERE
        ...
        AND EXISTS (
            ...
            WHERE us.user_id = tu.user_id
              ...
              AND (
                  -- Obtener la zona horaria del usuario, con fallback a la del tenant
                  SELECT COALESCE(au.raw_user_meta_data->>'timezone', v_tenant_timezone) INTO v_user_timezone
              )
              -- Convertir el horario del usuario (asumido en zona horaria del usuario) a UTC para comparación
              AND v_appointment_start_utc::TIME >= (us.start_time::TEXT || ':00')::TIME AT TIME ZONE v_user_timezone AT TIME ZONE 'UTC'::TIME
              AND v_appointment_end_utc::TIME <= (us.end_time::TEXT || ':00')::TIME AT TIME ZONE v_user_timezone AT TIME ZONE 'UTC'::TIME
        )
        AND NOT EXISTS (
            SELECT 1
