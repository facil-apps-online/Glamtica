-- Migración para corregir las funciones de matriz de comisiones de productos y servicios (v6).
...
BEGIN
  ...
  relevant_users AS (
    ...
    SELECT DISTINCT
        u.id as user_id,
        (u.raw_user_meta_data::jsonb->>'first_name') || ' ' || (u.raw_user_meta_data::jsonb->>'last_name') as user_name -- CAST explícito a jsonb
    FROM
        auth.users u,
