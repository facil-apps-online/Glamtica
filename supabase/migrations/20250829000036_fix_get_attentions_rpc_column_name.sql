-- First, drop the existing function to ensure a clean re-creation
DROP FUNCTION IF EXISTS public.get_attentions_with_details(uuid, uuid, uuid, text, timestamptz, timestamptz);

-- Then, create the corrected function with the correct column name 's.combo_id'
        ...
        (
            SELECT json_agg(
                json_build_object(
                    ...
                    'attention_combo_id', s.combo_id, -- CORRECTED COLUMN NAME
        ...
        (
            SELECT json_agg(
                json_build_object(
                    ...
                    'notes', p.notes,
                    'user_id', p.user_id,
                    'attention_combo_id', p.combo_id, -- CORRECTED COLUMN NAME
