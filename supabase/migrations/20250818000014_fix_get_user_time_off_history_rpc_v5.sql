...
BEGIN
    ...
    WHERE
        ...
        AND (
            ...
            (
                SELECT bool_and(word_match) FROM (
                    SELECT unnest(search_words) AS word
                ) AS words
                CROSS JOIN LATERAL (
                    SELECT 
                        tu.first_name ILIKE '%' || words.word || '%' OR
                        tu.last_name ILIKE '%' || words.word || '%' OR
                        tu.email ILIKE '%' || words.word || '%' OR
                        (tu.raw_user_meta_data->>'phone') ILIKE '%' || words.word || '%' as word_match
                ) AS matches
            )
        );
END;
