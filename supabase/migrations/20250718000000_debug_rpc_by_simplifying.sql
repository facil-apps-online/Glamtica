CREATE OR REPLACE FUNCTION get_tenants_with_metrics(search_term_param TEXT DEFAULT NULL)
RETURNS TABLE (
    id UUID,
    name TEXT,
    subscription_status TEXT,
    default_language_code TEXT,
    default_currency_id UUID,
    default_timezone TEXT,
    created_at TIMESTAMPTZ,
    updated_at TIMESTAMPTZ,
    country_id UUID,
    legal_name TEXT,
    tax_id TEXT,
    billing_address TEXT,
    website TEXT,
    contact_phone TEXT,
    whatsapp_phone TEXT,
    commercial_email TEXT,
    einvoicing_email TEXT,
    physical_address_line1 TEXT,
    physical_address_line2 TEXT,
    physical_city TEXT,
    physical_state TEXT,
    physical_postal_code TEXT,
    latitude REAL,
    longitude REAL,
    country_name TEXT
)
LANGUAGE plpgsql
AS $$
BEGIN
    -- Devolvemos directamente la consulta, comentando toda la lógica de métricas y tablas temporales para depurar.
    RETURN QUERY
    SELECT
        t.id, t.name, t.subscription_status, t.default_language_code, t.default_currency_id,
        t.default_timezone, t.created_at, t.updated_at, t.country_id, t.legal_name, t.tax_id,
        t.billing_address, t.website, t.contact_phone, t.whatsapp_phone, t.commercial_email,
        t.einvoicing_email, t.physical_address_line1, t.physical_address_line2, t.physical_city,
        t.physical_state, t.physical_postal_code, t.latitude, t.longitude,
        c.name as country_name
    FROM
        public.tenants t
    LEFT JOIN
        public.countries c ON t.country_id = c.id
    WHERE
        search_term_param IS NULL OR
        t.name ILIKE '%' || search_term_param || '%' OR
        t.legal_name ILIKE '%' || search_term_param || '%' OR
        t.commercial_email ILIKE '%' || search_term_param || '%' OR
        t.tax_id ILIKE '%' || search_term_param || '%'
    ORDER BY t.name ASC;

    /*
    -- LÓGICA DE MÉTRICAS TEMPORALMENTE DESHABILITADA PARA DEPURACIÓN
    DECLARE
        start_time TIMESTAMPTZ;
        end_time TIMESTAMPTZ;
        response_time_ms INTEGER;
    BEGIN
        start_time := clock_timestamp();

        CREATE TEMP TABLE temp_tenants AS
        SELECT ... ;

        end_time := clock_timestamp();
        response_time_ms := (EXTRACT(EPOCH FROM (end_time - start_time)) * 1000)::INTEGER;

        -- La línea que probablemente falla es la siguiente:
        INSERT INTO public.api_request_metrics (path, method, status_code, response_time_ms)
        VALUES ('rpc/get_tenants_with_metrics', 'POST', 200, response_time_ms);

        RETURN QUERY SELECT * FROM temp_tenants ORDER BY name ASC;

        DROP TABLE temp_tenants;
    */
END;
$$;