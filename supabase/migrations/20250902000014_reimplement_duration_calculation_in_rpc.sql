-- Migration to correctly implement total duration calculation in the get_attentions_with_details RPC.
...
BEGIN
    ...
    attention_durations AS (
        SELECT
            t.attention_id,
            COALESCE(MAX(t.end_offset), 0) AS calculated_duration
        FROM (
            SELECT
                s.attention_id,
                LAG(
                    SUM(CASE WHEN NOT s.is_parallel THEN COALESCE(s.duration_minutes, serv.duration_minutes, 0) ELSE 0 END) OVER (PARTITION BY s.attention_id ORDER BY s.service_order, s.id),
                    1, 0
                ) OVER (PARTITION BY s.attention_id ORDER BY s.service_order, s.id)
                + COALESCE(s.duration_minutes, serv.duration_minutes, 0) as end_offset
            FROM
                attention_services s
            JOIN
                services serv ON s.service_id = serv.id
            WHERE
                s.attention_id IN (SELECT id FROM base_attentions)
        ) t
        GROUP BY
            t.attention_id
    )
