-- Migración para corregir las funciones de matriz de comisiones de productos y servicios (v10).
...
BEGIN
  ...
  relevant_users AS (
    ...
    FROM
        auth.users u,
        jsonb_array_elements((u.raw_app_meta_data->'assignments')::jsonb) AS assignment -- Con CAST explícito en el input
    WHERE
        (assignment->'branch_id')::text::uuid IN (SELECT branch_id FROM relevant_branches)
        AND (assignment->'tenant_id')::text::uuid = tenant_id_param
  ),
