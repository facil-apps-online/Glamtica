-- Migration to fix attentions calendar view by supporting date ranges in the RPC

-- Step 1: Drop all possible old versions of the function to avoid signature conflicts.
-- The IF EXISTS clause prevents errors if a specific version doesn't exist.
DROP FUNCTION IF EXISTS get_attentions_with_details(uuid, uuid, uuid, text, date) CASCADE;
DROP FUNCTION IF EXISTS get_attentions_with_details(uuid, uuid, uuid, text, date, date) CASCADE;

-- Step 2: Create the new, definitive version of the function with start and end date parameters.
...
BEGIN
  ...
  WITH 
  tenant_users AS (
    SELECT * FROM public.get_tenant_users(p_tenant_id)
  ),
  attentions_filtered AS (
    ...
    WHERE
      ...
      AND (
        -- Handle date range for calendar view
        (p_start_date IS NOT NULL AND p_end_date IS NOT NULL AND a.attention_datetime::date BETWEEN p_start_date AND p_end_date)
        -- Handle single day for list view (fallback)
        OR (p_start_date IS NOT NULL AND p_end_date IS NULL AND a.attention_datetime::date = p_start_date)
        -- Handle case where no date filter is provided
  ...
  SELECT
    ...
    (SELECT json_agg(json_build_object(
        ...
        'services', (SELECT json_build_object('id', s.id, 'name', s.name) FROM public.services s WHERE s.id = aserv.service_id AND s.tenant_id = af.tenant_id LIMIT 1),
        'users', (SELECT json_build_object('id', tu.user_id, 'first_name', tu.first_name, 'last_name', tu.last_name) FROM tenant_users tu WHERE tu.user_id = aserv.user_id LIMIT 1)
    )) 
    ...
    (SELECT json_agg(json_build_object(
        ...
        'products', (SELECT json_build_object('id', p.id, 'name', p.name) FROM public.products p WHERE p.id = ap.product_id AND p.tenant_id = af.tenant_id),
        'users', (SELECT json_build_object('id', tu.user_id, 'first_name', tu.first_name, 'last_name', tu.last_name) FROM tenant_users tu WHERE tu.user_id = ap.user_id LIMIT 1)
    )) 
    ...
    (SELECT json_agg(json_build_object(
        ...
        'users', (SELECT json_build_object('id', tu.user_id, 'first_name', tu.first_name, 'last_name', tu.last_name) FROM tenant_users tu WHERE tu.user_id = ac.user_id LIMIT 1)
    )) 
