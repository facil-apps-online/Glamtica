-- Migración para crear la función RPC create_tenant_with_admin
-- Esta función crea un nuevo tenant y su usuario administrador principal
-- en una sola transacción para garantizar la integridad de los datos.
...
BEGIN
    ...
    -- 2. Obtener el ID del rol 'Tenant Superadmin' (asumimos que existe y es único)
    SELECT id INTO tenant_admin_role_id FROM roles WHERE name = 'Tenant Superadmin' AND tenant_id IS NULL LIMIT 1;
