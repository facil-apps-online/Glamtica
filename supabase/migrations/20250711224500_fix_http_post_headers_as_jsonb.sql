-- supabase/migrations/20250711224500_fix_http_post_headers_as_jsonb.sql

-- Recreate the function, passing headers as a jsonb object
...
DECLARE
    ...
    payload_to_edge_function TEXT;
    headers_jsonb JSONB; -- Declare a variable for JSONB headers
BEGIN
    ...
    payload_to_edge_function := json_build_object(
    ...
    -- Construct headers as a JSONB object
    headers_jsonb := jsonb_build_object('apikey', p_supabase_anon_key);

    -- Call Edge Function to generate JWT, passing headers as JSONB
    SELECT * FROM public.http_post(
        edge_function_url::character varying,
        payload_to_edge_function::character varying,
        'application/json'::character varying,
        headers_jsonb -- Pass the JSONB object for headers
