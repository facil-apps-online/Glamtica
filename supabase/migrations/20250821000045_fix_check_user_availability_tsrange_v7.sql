-- Migration: 20250821000045_fix_check_user_availability_tsrange_v7.sql
-- Description: Final fix for tsrange function call in check_user_availability by removing the explicit bounds text argument for TIMESTAMPTZ ranges.
...
BEGIN
    ...
    -- Crear el rango de la cita (sin el tercer argumento de texto, ya que [) es el default para TIMESTAMPTZ)
    v_appointment_range := TSRANGE(v_appointment_start_utc, v_appointment_end_utc);
    ...
    WHERE
        ...
        AND EXISTS (
            ...
            WHERE us.user_id = tu.user_id
              ...
              -- Crear el rango del horario del usuario en UTC (sin el tercer argumento de texto)
              AND TSRANGE(
                    (p_appointment_date::TEXT || ' ' || us.start_time::TEXT)::TIMESTAMP AT TIME ZONE COALESCE(au.raw_user_meta_data->>'timezone', v_tenant_timezone) AT TIME ZONE 'UTC',
                    (p_appointment_date::TEXT || ' ' || us.end_time::TEXT)::TIMESTAMP AT TIME ZONE COALESCE(au.raw_user_meta_data->>'timezone', v_tenant_timezone) AT TIME ZONE 'UTC'
