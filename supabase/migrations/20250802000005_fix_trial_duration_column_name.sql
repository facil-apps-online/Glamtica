-- Migration: 20250802000005_fix_trial_duration_column_name.sql
-- Description: Corrects the column name used to fetch the trial duration
-- inside the setup_tenant_for_new_user function from the non-existent
-- 'trial_duration_days' to the correct 'duration_days'.
...
BEGIN
    ...
    -- CORRECTED: Fetches from the correct 'duration_days' column.
...
COMMENT ON FUNCTION public.setup_tenant_for_new_user(UUID, UUID, JSONB) IS 'v4: Corrects the column name for trial duration lookup from trial_duration_days to duration_days.';
