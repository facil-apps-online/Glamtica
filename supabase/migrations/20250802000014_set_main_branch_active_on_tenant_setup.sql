...
COMMENT ON FUNCTION public.setup_tenant_for_new_user(UUID) IS 'v3: Canonical function to set up a complete tenant environment for a newly created user. Reads metadata from auth.users to perform setup, now including detailed branch creation and setting main branch to active.';
