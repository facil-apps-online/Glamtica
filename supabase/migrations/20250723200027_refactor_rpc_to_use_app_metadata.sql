-- MIGRATION: Update all branches RPCs to read claims from app_metadata

-- Helper function to get tenant_id from the new JWT structure
...
BEGIN
    ...
    IF v_tenant_id IS NULL THEN
        RAISE EXCEPTION 'Tenant ID not found in JWT app_metadata';
    END IF;
    RETURN v_tenant_id;
END;
$$;
...
AS $$
DECLARE
    v_tenant_id UUID := get_tenant_id_from_jwt();
