-- Migration: Fix get_attentions_with_details RPC to use timestamptz and date range

-- Step 1: Drop the old, outdated function.
DROP FUNCTION IF EXISTS get_attentions_with_details(uuid, uuid, uuid, text, date);
DROP FUNCTION IF EXISTS get_attentions_with_details(uuid, uuid, uuid, text, date, date); -- Drop if a version with date range exists
