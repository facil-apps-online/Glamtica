...
-- 1. Eliminar la función antigua que no es multi-plataforma.
DROP FUNCTION IF EXISTS public.get_superadmin_financial_stats();

-- 2. Crear la nueva función principal, parametrizada por plataforma.
...
BEGIN
    ...
    WITH valid_payments AS (
        ...
        SELECT 
            (pi.amount_in_cents / 100.0) as amount, -- Corregido: usa amount_in_cents y convierte a unidad principal
            pi.created_at,
            t.id as tenant_id,
        ...
        WHERE pi.status = 'COMPLETED'
          AND pi.environment <> 'test' -- Corregido: se filtra por entorno
...
-- 3. Crear la función para el dashboard de inversores.
