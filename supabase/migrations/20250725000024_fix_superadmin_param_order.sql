-- Migration: 20250725000024_fix_superadmin_param_order.sql
-- Description: Corrects the parameter order of the create_application_superadmin function
-- to match the signature expected by the Supabase client RPC call.

-- Step 1: Drop the function with the incorrect parameter order.
-- The function signature is defined by the types, but we drop by name and parameter types
-- to be explicit.
DROP FUNCTION IF EXISTS public.create_application_superadmin(TEXT, TEXT);

-- Step 2: Recreate the function with the correct parameter order.
-- The order is now (p_admin_email TEXT, p_application_name TEXT) to match the client call.
...
-- COMMENT: Corrects the parameter order for the initial superadmin setup RPC.
