-- Migration: Correctly enriches the get_attentions_with_details RPC to include user details in services and products.
...
-- Create the new, enriched function.
...
BEGIN
  ...
  SELECT
    ...
    (SELECT json_agg(json_build_object(
        ...
        'users', (SELECT json_build_object('first_name', u.raw_user_meta_data->>'first_name', 'last_name', u.raw_user_meta_data->>'last_name') FROM auth.users u WHERE u.id = aserv.user_id)
    )) 
    FROM public.attention_services aserv WHERE aserv.attention_id = af.id) as attention_services,
    
    -- Enriched attention_products
    (SELECT json_agg(json_build_object(
        'id', ap.id, 
        ...
        'users', (SELECT json_build_object('first_name', u.raw_user_meta_data->>'first_name', 'last_name', u.raw_user_meta_data->>'last_name') FROM auth.users u WHERE u.id = ap.user_id)
    )) 
    FROM public.attention_products ap WHERE ap.attention_id = af.id) as attention_products,
    
    -- attention_combos remains the same as it does not have a direct user_id
    (SELECT json_agg(json_build_object(
        'id', ac.id, 
