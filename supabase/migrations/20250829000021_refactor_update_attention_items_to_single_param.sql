-- Migration to refactor update_attention_items to use a single JSONB parameter for robustness

-- Step 1: Drop all previous known signatures to ensure a clean state
...
-- Step 2: Create the new function with a single JSONB parameter
...
BEGIN
  ...
  LOOP
    INSERT INTO public.attention_services (id, attention_id, service_id, user_id, service_price, duration_minutes, start_time, end_time, status, is_parallel, parallel_group_id, offset_minutes, notes, tenant_id, branch_id)
    VALUES (
      COALESCE((service_item->>'id')::uuid, gen_random_uuid()), p_attention_id, (service_item->>'service_id')::uuid, (service_item->>'user_id')::uuid,
      (service_item->>'price')::numeric, (service_item->>'duration')::integer, (service_item->>'start_time')::time, (service_item->>'end_time')::time,
      service_item->>'status', (service_item->>'is_parallel')::boolean, (service_item->>'parallel_group_id')::uuid,
      (service_item->>'offset_minutes')::integer, service_item->>'notes', p_tenant_id, p_branch_id
    ) ON CONFLICT (id) DO UPDATE SET
      service_id = (service_item->>'service_id')::uuid, user_id = (service_item->>'user_id')::uuid, service_price = (service_item->>'price')::numeric,
      duration_minutes = (service_item->>'duration')::integer, start_time = (service_item->>'start_time')::time, end_time = (service_item->>'end_time')::time,
      status = service_item->>'status', is_parallel = (service_item->>'is_parallel')::boolean, parallel_group_id = (service_item->>'parallel_group_id')::uuid,
      offset_minutes = (service_item->>'offset_minutes')::integer, notes = service_item->>'notes';
  END LOOP;
  ...
  LOOP
    ...
    VALUES (
      ...
      product_id = (product_item->>'product_id')::uuid, user_id = (product_item->>'commission_user_id')::uuid, quantity = (product_item->>'quantity')::integer,
      unit_price = (product_item->>'unit_price')::numeric, total_price = (product_item->>'quantity')::integer * (product_item->>'unit_price')::numeric;
  END LOOP;

  -- 4. Upsert combos AND THEIR ITEMS
  FOR combo_item IN SELECT * FROM jsonb_array_elements(p_combos_to_upsert)
  LOOP
    INSERT INTO public.attention_combos (id, attention_id, combo_id, user_id, price, quantity, status, notes, tenant_id, branch_id)
    VALUES (
      COALESCE((combo_item->>'id')::uuid, gen_random_uuid()), p_attention_id, (combo_item->>'combo_id')::uuid, (combo_item->>'user_id')::uuid,
      (combo_item->>'price')::numeric, (combo_item->>'quantity')::integer, combo_item->>'status', combo_item->>'notes', p_tenant_id, p_branch_id
    ) ON CONFLICT (id) DO UPDATE SET
      combo_id = (combo_item->>'combo_id')::uuid, user_id = (combo_item->>'user_id')::uuid, price = (combo_item->>'price')::numeric,
      quantity = (combo_item->>'quantity')::integer, status = combo_item->>'status', notes = combo_item->>'notes'
    ...
    IF (combo_item->>'id') IS NULL THEN
      ...
      LOOP
        IF v_combo_item.service_id IS NOT NULL THEN
          ...
          INSERT INTO public.attention_services(attention_id, service_id, user_id, service_price, notes, tenant_id, branch_id, duration_minutes, combo_id, status, start_time, end_time, is_parallel, parallel_group_id, offset_minutes)
          VALUES (p_attention_id, v_combo_item.service_id, (combo_item->>'user_id')::uuid, COALESCE(v_service_data.selling_price, 0), combo_item->>'notes', p_tenant_id, p_branch_id, COALESCE(v_service_data.duration_minutes, 0), v_attention_combo_id, 'Pendiente', (combo_item->>'start_time')::time, (combo_item->>'end_time')::time, (combo_item->>'is_parallel')::boolean, (combo_item->>'parallel_group_id')::uuid, (combo_item->>'offset_minutes')::integer);
        END IF;
        IF v_combo_item.product_id IS NOT NULL THEN
          ...
          VALUES (p_attention_id, v_combo_item.product_id, (combo_item->>'user_id')::uuid, v_combo_item.quantity, COALESCE(v_branch_product.selling_price, 0), COALESCE(v_branch_product.selling_price, 0) * v_combo_item.quantity, p_tenant_id, p_branch_id, v_attention_combo_id);
        END IF;
      END LOOP;
    END IF;
  END LOOP;
