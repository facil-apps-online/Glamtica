...
BEGIN
    ...
    current_tariffs AS (
        SELECT DISTINCT ON (pt.subscription_plan_id)
            ...
            (SELECT pal.value::INT FROM public.plan_asset_limits pal JOIN assets a ON pal.asset_id = a.id WHERE pal.plan_id = sp.id AND a.asset_key = 'fe_glam' AND pal.country_id = p_country_id LIMIT 1) AS included_einvoices,
            (SELECT pal.extra_unit_price FROM public.plan_asset_limits pal JOIN assets a ON pal.asset_id = a.id WHERE pal.plan_id = sp.id AND a.asset_key = 'fe_glam' AND pal.country_id = p_country_id LIMIT 1) AS extra_einvoice_price
