-- Migration: Fix ambiguous column reference in get_user_time_off_history by aliasing CTE columns.

-- Step 1: Drop the potentially faulty function to ensure a clean slate.
DROP FUNCTION IF EXISTS get_user_time_off_history(uuid, uuid, text, text, date, date, uuid, text);

-- Step 2: Create the corrected function with aliased columns in the CTE.
