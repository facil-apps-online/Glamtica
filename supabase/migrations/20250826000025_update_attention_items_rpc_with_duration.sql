-- Migration: Update update_attention_items RPC to handle duration and parallel fields
...
BEGIN
    ...
    IF array_length(p_service_ids_to_delete, 1) IS NOT NULL THEN
        ...
        WHERE attention_id = p_attention_id
          ...
          AND status = 'Pendiente'; -- Only pending services can be deleted
    END IF;
    ...
    IF jsonb_array_length(p_services_to_upsert) > 0 THEN
        ...
        LOOP
            INSERT INTO public.attention_services (id, attention_id, service_id, user_id, service_price, notes, status, tenant_id, branch_id, duration_minutes, start_time, end_time, is_parallel, parallel_group_id)
            VALUES (
    ...
    LEFT JOIN
        public.attention_services aserv ON att.id = aserv.attention_id
    LEFT JOIN
        public.attention_products aprod ON att.id = aprod.attention_id
    WHERE
        att.id = p_attention_id;
