-- Migration to finally and definitively fix the ambiguous id reference in get_attentions_with_details RPC.
    ...
    attention_durations AS (
        WITH service_details AS (
            SELECT 
                ...
                s.service_order,
                s.id as attention_service_id -- Explicitly aliased to avoid ambiguity
            FROM attention_services s
            JOIN services serv ON s.service_id = serv.id
            WHERE s.attention_id IN (SELECT ba.id FROM base_attentions)
        ),
        timeline AS (
            SELECT
                attention_id,
