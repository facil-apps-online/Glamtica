-- Migración para corregir las funciones de matriz de comisiones de productos y servicios para usar get_tenant_users RPC.
...
BEGIN
  ...
  relevant_users AS (
    ...
    --    Obtenemos los usuarios del RPC get_tenant_users y filtramos por sucursal.
    SELECT DISTINCT
        gtu.user_id,
        gtu.first_name,
        gtu.last_name
    ...
    WHERE
        gtu.branch_id IN (SELECT branch_id FROM relevant_branches)
  ),
  ...
  SELECT
    cm.user_id,
    cm.first_name || ' ' || cm.last_name as user_name,
    json_agg(
      json_build_object(
