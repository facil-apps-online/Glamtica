-- Migration: 20250821000043_fix_check_user_availability_tsrange_final.sql
-- Description: Final fix for tsrange function call in check_user_availability using TIMESTAMPTZ ranges.
    ...
    WHERE
        ...
        AND EXISTS (
            ...
            WHERE us.user_id = tu.user_id
              ...
              -- Crear un rango de tiempo para el horario del usuario en UTC
              AND TSRANGE(
                    ...
                    '[)'::TEXT -- Incluye el inicio, excluye el final
                  ) @> v_appointment_start_utc -- El horario del usuario debe contener el inicio de la cita
              AND TSRANGE(
                    (p_appointment_date::TEXT || ' ' || us.start_time::TEXT)::TIMESTAMP AT TIME ZONE COALESCE(au.raw_user_meta_data->>'timezone', v_tenant_timezone) AT TIME ZONE 'UTC',
                    (p_appointment_date::TEXT || ' ' || us.end_time::TEXT)::TIMESTAMP AT TIME ZONE COALESCE(au.raw_user_meta_data->>'timezone', v_tenant_timezone) AT TIME ZONE 'UTC',
                    '[)'::TEXT
                  ) @> v_appointment_end_utc -- Y el horario del usuario debe contener el final de la cita
        )
        AND NOT EXISTS (
            SELECT 1
