    ...
    attention_durations AS (
        WITH service_details AS (
            SELECT 
                ...
                s.service_order,
                s.id -- This is attention_services.id. No other 'id' is in this SELECT's scope.
        ...
        timeline AS (
            SELECT
                attention_id,
                LAG(
                    SUM(CASE WHEN NOT is_parallel THEN duration_minutes ELSE 0 END) OVER (PARTITION BY attention_id ORDER BY service_order, id), -- Now 'id' can only refer to s.id
                    1, 0
                ) OVER (PARTITION BY attention_id ORDER BY service_order, id) 
