...
-- Step 1: Add a tsvector column to the clients table
ALTER TABLE public.clients ADD COLUMN fts tsvector;

-- Step 2: Create a function to update the tsvector column
CREATE OR REPLACE FUNCTION update_clients_fts() RETURNS TRIGGER AS $$
BEGIN
...
-- Step 3: Create a trigger to automatically update the tsvector column
CREATE TRIGGER clients_fts_update
BEFORE INSERT OR UPDATE ON public.clients
FOR EACH ROW EXECUTE FUNCTION update_clients_fts();

-- Step 4: Update existing rows
UPDATE public.clients SET fts = to_tsvector('simple', 
    coalesce(name, '') || ' ' || 
    coalesce(email, '') || ' ' || 
    coalesce(phone, '') || ' ' || 
...
-- Step 5: Update the search_clients function
...
BEGIN
    ...
    IF p_search_term IS NOT NULL AND p_search_term <> '' THEN
        search_query := string_to_array(lower(p_search_term), ' ');
        search_query := array_to_string(search_query, ' & ');
    ELSE
        search_query := NULL;
    END IF;
    ...
    IF search_query IS NOT NULL THEN
        final_query := final_query || '
            AND c.fts @@ to_tsquery(''simple'', ''' || search_query || ''')';
    END IF;
