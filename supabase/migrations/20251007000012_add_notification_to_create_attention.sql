...
BEGIN
    ...
    VALUES (p_client_id, p_attention_datetime, p_notes, p_total_amount, p_tenant_id, p_branch_id, 'Pendiente')
    ...
    -- Enqueue client notification email for the new attention
    BEGIN
        PERFORM public.queue_client_email(
            p_attention_id := v_attention_id,
            p_tenant_id := p_tenant_id,
            p_template_name := 'new_attention'
        );
    EXCEPTION
        WHEN others THEN
            -- Log the error but do not fail the transaction
            RAISE WARNING 'Failed to queue client email for attention_id %: %', v_attention_id, SQLERRM;
    END;
