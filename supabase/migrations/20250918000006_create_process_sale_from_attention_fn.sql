-- 1. Drop the old function if it exists, as we are replacing it.
DROP FUNCTION IF EXISTS public.generate_invoice_for_attention(uuid);

-- 2. Create the new, comprehensive function to process a sale from an attention.
...
BEGIN
    ...
    -- Step 3: Get the next sale number
    sale_number_new := public.get_next_document_number(attention_rec.tenant_id, 'SALE', attention_rec.branch_id);
    ...
    -- Process Combos (only if services are being invoiced)
    IF (tenant_settings->>'invoice_services')::boolean THEN
        ...
        LOOP
            -- Insert the combo parent item (visual grouping, no price)
            ...
            -- Process services within this combo
            FOR item IN
            ...
            LOOP
                -- NOTE: Tax calculation logic would be added here. For now, it's 0.
            ...
            -- Process products within this combo (only if products are being invoiced)
            IF (tenant_settings->>'invoice_products')::boolean THEN
                ...
                LOOP
                    -- NOTE: Tax calculation logic would be added here.
                    ...
                    -- Get current cost for the movement record
                    ...
                    PERFORM public.create_product_movement(attention_rec.tenant_id, attention_rec.branch_id, item.product_id, 'SALE', -item.quantity, product_cost, sale_id_new, 'SALE');
                END LOOP;
            END IF;
        END LOOP;
    END IF;
    ...
    -- Step 6: Calculate final totals from all inserted sales_items
    SELECT
    ...
    -- Step 7: Update the main 'sales' record with the correct totals
    UPDATE public.sales
    SET
    ...
    -- Step 8: Return the ID of the newly created sale
    RETURN sale_id_new;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION public.process_sale_from_attention IS 'Orchestrates the creation of a sale record from an attention, including hierarchical items, inventory movements, and numbering.';
