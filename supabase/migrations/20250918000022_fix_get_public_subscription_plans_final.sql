DROP FUNCTION public.get_public_subscription_plans(UUID, UUID);
...
RETURNS TABLE (
    ...
    included_einvoices INT,
    extra_einvoice_price NUMERIC,
    extra_branch_bonus_einvoices INT
)
LANGUAGE plpgsql
AS $$
BEGIN
    ...
    current_tariffs AS (
        SELECT DISTINCT ON (pt.subscription_plan_id)
            ...
            (SELECT tap.extra_unit_price FROM public.tariff_asset_prices tap JOIN assets a ON tap.asset_id = a.id WHERE tap.tariff_id = pt.id AND a.asset_key = 'suc_glam' LIMIT 1) AS extra_branch_price,
            (SELECT pal.value::INT FROM public.plan_asset_limits pal JOIN assets a ON pal.asset_id = a.id WHERE pal.plan_id = pt.subscription_plan_id AND a.asset_key = 'fe_glam' AND pal.country_id = p_country_id LIMIT 1) AS included_einvoices,
            (SELECT pal.extra_unit_price FROM public.plan_asset_limits pal JOIN assets a ON pal.asset_id = a.id WHERE pal.plan_id = pt.subscription_plan_id AND a.asset_key = 'fe_glam' AND pal.country_id = p_country_id LIMIT 1) AS extra_einvoice_price,
            (SELECT (pal.bonus_on_extra->>'quantity')::INT FROM public.plan_asset_limits pal JOIN assets a ON pal.asset_id = a.id WHERE pal.plan_id = pt.subscription_plan_id AND a.asset_key = 'suc_glam' AND pal.country_id = p_country_id LIMIT 1) AS extra_branch_bonus_einvoices
    ...
    SELECT
        ...
        (CASE
            ...
            ELSE floor(ct.base_price::numeric * (SELECT rate::numeric FROM rates_to_usd WHERE base_currency_code = ct.base_currency_code LIMIT 1) * cr.usd_to_target_rate::numeric) + 0.99
        END)::numeric AS calculated_price,
        (CASE
            WHEN ct.base_currency_code = cr.ccode THEN ct.extra_branch_price
            ELSE floor(ct.extra_branch_price::numeric * (SELECT rate::numeric FROM rates_to_usd WHERE base_currency_code = ct.base_currency_code LIMIT 1) * cr.usd_to_target_rate::numeric) + 0.99
        END)::numeric AS calculated_extra_branch_price,
        (CASE
            WHEN ct.base_currency_code = cr.ccode THEN COALESCE(ct.promotional_price, 0)
            ELSE floor(COALESCE(ct.promotional_price, 0)::numeric * (SELECT rate::numeric FROM rates_to_usd WHERE base_currency_code = ct.base_currency_code LIMIT 1) * cr.usd_to_target_rate::numeric) + 0.99
        ...
        (CASE
            ...
            ELSE floor(ct.extra_einvoice_price::numeric * (SELECT rate::numeric FROM rates_to_usd WHERE base_currency_code = ct.base_currency_code LIMIT 1) * cr.usd_to_target_rate::numeric) + 0.99
