-- MIGRATION: Add invoice generated notification
...
BEGIN
    ...
    IF v_invoice_id IS NOT NULL THEN
        -- Enqueue client notification email for the existing invoice
        BEGIN
            PERFORM public.queue_client_email(
                p_attention_id := p_attention_id,
                p_tenant_id := v_tenant_id,
                p_template_name := 'invoice_generated'
            );
        EXCEPTION
            WHEN others THEN
                RAISE WARNING 'Failed to queue client invoice email for attention_id %: %', p_attention_id, SQLERRM;
        END;
        RETURN v_invoice_id;
    END IF;
    ...
    -- Enqueue client notification email for the new invoice
    BEGIN
        PERFORM public.queue_client_email(
            p_attention_id := p_attention_id,
            p_tenant_id := v_tenant_id,
