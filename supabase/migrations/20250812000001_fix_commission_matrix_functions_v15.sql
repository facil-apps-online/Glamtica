-- Migración para corregir las funciones de matriz de comisiones de productos y servicios (v12).
...
BEGIN
  ...
  relevant_users AS (
    ...
    FROM
        auth.users u,
        jsonb_array_elements((u.raw_app_meta_data->'assignments')::jsonb) AS assignment
    WHERE
        ((assignment)::jsonb->>'branch_id')::uuid IN (SELECT branch_id FROM relevant_branches)
        AND ((assignment)::jsonb->>'tenant_id')::uuid = tenant_id_param
  ),
  ...
  GROUP BY cm.user_id, cm.first_name, cm.last_name;
END;
$$ LANGUAGE plpgsql;
