-- V5 of the function, fixing tenant settings keys.
...
BEGIN
    ...
    IF tenant_settings IS NULL THEN
        tenant_settings := '{"invoice_products_enabled": true, "invoice_services_enabled": true}';
    END IF;
    ...
    -- Step 4: Create the main 'sales' record
    ...
    -- Step 5: Process and insert sale items
    IF (tenant_settings->>'invoice_services_enabled')::boolean THEN
        FOR combo_item IN
        ...
        LOOP
            ...
            IF (tenant_settings->>'invoice_products_enabled')::boolean THEN
                FOR product_item IN
...
COMMENT ON FUNCTION public.process_sale_from_attention IS 'V5: Fix tenant settings keys to use _enabled suffix.';
