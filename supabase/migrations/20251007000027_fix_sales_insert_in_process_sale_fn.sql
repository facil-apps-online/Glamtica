-- V10 of the function, fixing a NOT NULL violation on sales insert.
...
BEGIN
    ...
    EXCEPTION WHEN others THEN RAISE WARNING 'V10: Failed to queue client receipt email for attention_id %: %', p_attention_id, SQLERRM;
    END;
    BEGIN
        PERFORM public.queue_client_whatsapp(attention_rec.tenant_id, attention_rec.client_id, 'payment_receipt_whatsapp', '{}'::jsonb);
    EXCEPTION WHEN others THEN RAISE WARNING 'V10: Failed to queue client receipt WhatsApp for attention_id %: %', p_attention_id, SQLERRM;
    END;
...
COMMENT ON FUNCTION public.process_sale_from_attention IS 'V10: Fix NOT NULL violation on sales insert by providing initial zero values for totals.';
