-- Corregir la función de login añadiendo SECURITY DEFINER
-- Esto es necesario para que la función pueda acceder a las tablas de usuarios y asignaciones
-- con los permisos del creador de la función, en lugar de los permisos del usuario que la invoca (que es 'anon' y no tiene permisos).
...
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER -- <<< CORRECCIÓN CLAVE
AS $$
DECLARE
    user_record RECORD;
...
BEGIN
    ...
    -- 2. Nueva comprobación: Verificar si el usuario tiene al menos una asignación activa
    SELECT EXISTS (
        SELECT 1
    ...
    -- 3. Si todo es correcto, obtener todas las asignaciones del usuario para devolverlas
    SELECT jsonb_agg(
        jsonb_build_object(
            'assignment_id', ua.id,
