-- Migration to align the audit trigger function for `products` with the one for `services`.
-- This reverts to the exact implementation that is known to work for the `services` table,
-- without extra type casts, to resolve the "function does not exist" error.
...
BEGIN
    ...
    -- Call the central logging function, exactly as it's done for services
    PERFORM public.log_audit_action(
        ...
        p_branch_id := NULL -- No explicit cast
    );
