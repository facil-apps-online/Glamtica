-- V8 of the function, adding WhatsApp receipt notification and fixing email queue parameters.
...
BEGIN
    ...
    -- Step 7: Enqueue client notifications for the payment receipt
    BEGIN
        -- Email Notification
        PERFORM public.queue_client_email(
            p_tenant_id := attention_rec.tenant_id,
            p_client_id := attention_rec.client_id,
            p_template_type := 'payment_receipt',
            p_template_data := '{}'::jsonb
        );
    EXCEPTION
        WHEN others THEN
            RAISE WARNING 'Failed to queue client receipt email for attention_id %: %', p_attention_id, SQLERRM;
    END;
    ...
    BEGIN
        ...
        PERFORM public.queue_client_whatsapp(
            p_tenant_id := attention_rec.tenant_id,
            p_client_id := attention_rec.client_id,
            p_template_name := 'payment_receipt_whatsapp',
            p_template_params := '{}'::jsonb
        );
    EXCEPTION
        WHEN others THEN
            RAISE WARNING 'Failed to queue client receipt WhatsApp for attention_id %: %', p_attention_id, SQLERRM;
    END;
...
COMMENT ON FUNCTION public.process_sale_from_attention IS 'V8: Add WhatsApp receipt notification and fix email queue parameters.';