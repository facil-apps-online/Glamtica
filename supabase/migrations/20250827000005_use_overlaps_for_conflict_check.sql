-- Migration: Revert to using OVERLAPS for robust appointment conflict checking

-- Step 1: Drop the existing function with the faulty explicit time comparison.
DROP FUNCTION IF EXISTS public.check_user_availability(UUID, TEXT, UUID, UUID, TIMESTAMPTZ, INTEGER, UUID, UUID);

-- Step 2: Recreate the function using the standard SQL OVERLAPS operator.
    ...
    WHERE
        ...
        AND (
            ( -- Availability logic
                ...
                -- Check for conflicting attentions (CORRECTED with OVERLAPS)
                AND NOT EXISTS (
                    SELECT 1
                    ...
                    WHERE aserv.user_id = tu.user_id
                      ...
                      AND (a.attention_datetime, a.attention_datetime + (aserv.duration_minutes || ' minutes')::interval) 
                          OVERLAPS 
                          (p_appointment_datetime, v_appointment_end_utc)
                )
                -- Check for time off (CORRECTED with OVERLAPS)
                AND NOT EXISTS (
                    SELECT 1
                    ...
                    WHERE uto.user_id = tu.user_id
                      ...
                      AND (uto.start_date, uto.end_date) 
                          OVERLAPS 
                          (p_appointment_datetime, v_appointment_end_utc)
                )
            )
