-- Migration: Fix get_attentions_with_details to include user details using get_tenant_users RPC.
-- This fixes a permission denied error by not querying auth.users directly.
...
BEGIN
  ...
  WITH 
    tenant_users AS (
      SELECT gtu.user_id, gtu.first_name, gtu.last_name FROM get_tenant_users(p_tenant_id) gtu
    ),
