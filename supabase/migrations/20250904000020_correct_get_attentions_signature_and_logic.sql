-- Migration: Corrects the get_attentions_with_details function with the proper signature (start/end dates) and removes the reference to the deleted user_id column.

-- Drop the old function with the incorrect signature to prevent overloading issues.
DROP FUNCTION IF EXISTS public.get_attentions_with_details(uuid, uuid, uuid, text, date) CASCADE;

-- Create the new, correct function that matches the Edge Function's call signature.
