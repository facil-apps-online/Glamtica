-- Migration: Fixes get_attentions_with_details to securely fetch user data via a JOIN with user_assignments, preventing a permissions error.

-- Drop the old function to replace it.
DROP FUNCTION IF EXISTS public.get_attentions_with_details(uuid, uuid, uuid, text, date, date) CASCADE;

-- Create the new, secure, and correct function.
...
BEGIN
  ...
  SELECT
    ...
    -- Enriched attention_services with secure user data fetching
    (SELECT json_agg(json_build_object(
        ...
        'users', (SELECT json_build_object('first_name', u.raw_user_meta_data->>'first_name', 'last_name', u.raw_user_meta_data->>'last_name') FROM auth.users u JOIN public.user_assignments ua ON u.id = ua.user_id WHERE ua.user_id = aserv.user_id AND ua.tenant_id = p_tenant_id LIMIT 1)
    )) 
    FROM public.attention_services aserv WHERE aserv.attention_id = af.id) as attention_services,
    
    -- Enriched attention_products with secure user data fetching
    (SELECT json_agg(json_build_object(
        'id', ap.id, 
        ...
        'users', (SELECT json_build_object('first_name', u.raw_user_meta_data->>'first_name', 'last_name', u.raw_user_meta_data->>'last_name') FROM auth.users u JOIN public.user_assignments ua ON u.id = ua.user_id WHERE ua.user_id = ap.user_id AND ua.tenant_id = p_tenant_id LIMIT 1)
    )) 
    FROM public.attention_products ap WHERE ap.attention_id = af.id) as attention_products,
    
    -- attention_combos remains the same
    (SELECT json_agg(json_build_object(
        'id', ac.id, 
