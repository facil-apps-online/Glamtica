-- Migration: 20250725000028_unify_architecture_and_finalize_setup.sql
-- Description: This is a definitive migration that unifies the database architecture.
-- It removes conflicting constraints, repairs existing data, establishes the roles table,
-- and creates the final, correct version of the initial setup function.

-- Step 1: Remove the conflicting CHECK constraint that was causing issues.
ALTER TABLE public.tenants DROP CONSTRAINT IF EXISTS platform_id_not_null_for_regular_tenants;

-- Step 2: Repair any existing tenant data that might have a NULL platform_id.
-- This ensures all tenants conform to the new universal rule.
UPDATE public.tenants
SET platform_id = (SELECT id FROM public.platforms LIMIT 1)
WHERE platform_id IS NULL;

-- Step 3: Establish the simple, universal rule: every tenant MUST have a platform.
ALTER TABLE public.tenants ALTER COLUMN platform_id SET NOT NULL;

-- Step 4: Create the roles table as the single source of truth for authorization.
...
-- Step 5: Seed the roles table with the defined roles for the system.
...
-- Step 6: Recreate the check_superadmin_exists function to use the new roles table.
...
-- Step 7: Recreate the create_application_superadmin function with the final, correct logic.
...
-- COMMENT: Unifies the database architecture and finalizes the initial setup RPC.
