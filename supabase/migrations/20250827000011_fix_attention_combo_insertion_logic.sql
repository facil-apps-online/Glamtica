...
-- Add notes column to attention_combos table
ALTER TABLE public.attention_combos
ADD COLUMN notes TEXT;

-- Recreate the create_full_attention function to handle notes and unnest combo items
...
BEGIN
    ...
    IF jsonb_array_length(p_services) > 0 THEN
        ...
        LOOP
            INSERT INTO public.attention_services (attention_id, service_id, user_id, service_price, notes, tenant_id, branch_id, duration, start_time, end_time)
            VALUES (v_attention_id, (v_service->>'service_id')::uuid, (v_service->>'user_id')::uuid, COALESCE((v_service->>'price')::numeric, 0), v_service->>'notes', p_tenant_id, p_branch_id, (v_service->>'duration')::integer, (v_service->>'start_time')::time, (v_service->>'end_time')::time);
        END LOOP;
    END IF;
    ...
    IF jsonb_array_length(p_combos) > 0 THEN
        ...
        LOOP
            ...
            INSERT INTO public.attention_combos (attention_id, combo_id, user_id, price, quantity, notes, tenant_id, branch_id, status)
            VALUES (v_attention_id, (v_combo->>'combo_id')::uuid, (v_combo->>'user_id')::uuid, COALESCE((v_combo->>'price')::numeric, 0), COALESCE((v_combo->>'quantity')::integer, 1), v_combo->>'notes', p_tenant_id, p_branch_id, 'Pendiente')
            ...
            LOOP
                -- If the combo item is a service
                IF v_combo_item.service_id IS NOT NULL THEN
                    -- Get the current price and duration from branch_services
                    SELECT selling_price, duration_minutes INTO v_branch_service 
                    FROM public.branch_services 
                    WHERE service_id = v_combo_item.service_id AND branch_id = p_branch_id;

                    INSERT INTO public.attention_services(attention_id, service_id, user_id, service_price, notes, tenant_id, branch_id, duration, combo_id, status)
                    VALUES (v_attention_id, v_combo_item.service_id, (v_combo->>'user_id')::uuid, COALESCE(v_branch_service.selling_price, 0), v_combo->>'notes', p_tenant_id, p_branch_id, COALESCE(v_branch_service.duration_minutes, 0), v_attention_combo_id, 'Pendiente');
                END IF;

                -- If the combo item is a product
                IF v_combo_item.product_id IS NOT NULL THEN
                    -- Get the current price from branch_products
