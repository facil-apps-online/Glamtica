-- Migration: 20250821000044_fix_check_user_availability_tsrange_v6.sql
-- Description: Final fix for tsrange operator in check_user_availability using range containment.
...
BEGIN
    ...
    -- Crear el rango de la cita
    v_appointment_range := TSRANGE(v_appointment_start_utc, v_appointment_end_utc, '[)'::TEXT);
    ...
    WHERE
        ...
        AND EXISTS (
            ...
            WHERE us.user_id = tu.user_id
              ...
              -- Crear el rango del horario del usuario en UTC
              AND TSRANGE(
            ...
            WHERE uto.user_id = tu.user_id
              ...
              AND (
                  ...
                  (uto.is_partial_day AND v_appointment_start_utc::DATE = uto.start_date::DATE AND
                   TSRANGE(v_appointment_start_utc, v_appointment_end_utc, '[)'::TEXT) && TSRANGE(uto.start_date, uto.end_date, '[)'::TEXT))
              )
        );
END;
$$;