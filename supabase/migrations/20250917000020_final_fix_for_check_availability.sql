...
-- MIGRATION: Definitive fix for check_user_availability, using branch timezone.
...
BEGIN
    -- 1. Get the branch timezone, falling back to tenant timezone, then UTC.
    SELECT b.timezone INTO v_branch_timezone FROM public.branches b WHERE b.id = p_branch_id;
    IF v_branch_timezone IS NULL THEN
        SELECT t.default_timezone INTO v_branch_timezone FROM public.tenants t WHERE t.id = p_tenant_id;
        IF v_branch_timezone IS NULL THEN
    ...
    -- The input p_appointment_datetime is a string from the client, which we interpret in the branch's timezone.
            ...
            ( -- Main availability logic block
                EXISTS (
                    ...
                    WHERE us.user_id = ua.user_id
                      ...
                      AND TSRANGE(
                            ( (p_appointment_datetime::DATE::TEXT || ' ' || us.start_time::TEXT)::TIMESTAMP AT TIME ZONE COALESCE((u.raw_user_meta_data ->> 'timezone')::text, v_branch_timezone) AT TIME ZONE 'UTC' )::TIMESTAMP WITHOUT TIME ZONE,
                            ( (p_appointment_datetime::DATE::TEXT || ' ' || us.end_time::TEXT)::TIMESTAMP AT TIME ZONE COALESCE((u.raw_user_meta_data ->> 'timezone')::text, v_branch_timezone) AT TIME ZONE 'UTC' )::TIMESTAMP WITHOUT TIME ZONE
                          ) @> v_appointment_range
                )
                AND NOT EXISTS (
                    SELECT 1
