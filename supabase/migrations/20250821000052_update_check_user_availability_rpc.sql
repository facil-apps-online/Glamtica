CREATE OR REPLACE FUNCTION public.check_user_availability(
    ...
    p_assigned_user_id UUID DEFAULT NULL -- NUEVO PARÁMETRO
)
RETURNS TABLE(
    user_id UUID,
...
BEGIN
    ...
    WHERE
        ...
        AND (
            ( -- Lógica de disponibilidad existente
                EXISTS (
                    SELECT 1
            ...
            OR (tu.user_id = p_assigned_user_id AND p_assigned_user_id IS NOT NULL) -- NUEVA CONDICIÓN
        );
END;
$$;