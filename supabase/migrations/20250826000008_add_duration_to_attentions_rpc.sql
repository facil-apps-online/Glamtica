-- Migration: Add duration_minutes to get_attentions_with_details RPC
-- This adds the duration_minutes field to both attention_services and services within combo_items.

-- Step 1: Drop the old function.
DROP FUNCTION IF EXISTS get_attentions_with_details(uuid, uuid, uuid, text, date, date) CASCADE;

-- Step 2: Create the new function with the added duration_minutes fields.
...
BEGIN
  ...
  WITH 
    tenant_users AS (
      SELECT DISTINCT ON (gtu.user_id) gtu.user_id, gtu.first_name, gtu.last_name FROM get_tenant_users(p_tenant_id) gtu
    ),
    attentions_filtered AS (
      ...
      WHERE
        ...
        AND (p_date_range_start IS NULL OR a.attention_date >= p_date_range_start)
        AND (p_date_range_end IS NULL OR a.attention_date <= p_date_range_end)
  ...
  SELECT
    ...
    af.attention_date,
    af.attention_time,
    (af.attention_date + af.attention_time)::timestamp with time zone as attention_datetime,
    af.status,
    af.notes,
    (
    ...
    (
      SELECT json_agg(
        json_build_object(
          ...
          'duration_minutes', aserv.duration_minutes, -- <<< ADDED
          'services', (
            SELECT json_build_object(
              'id', s.id,
              'name', s.name,
              'duration_minutes', s.duration_minutes -- <<< ADDED
            )
              ...
              'combo_items', (
                SELECT json_agg(
                  json_build_object(
                    ...
                    'service', (SELECT json_build_object('name', s.name, 'duration_minutes', s.duration_minutes) FROM public.services s WHERE s.id = ci.service_id) -- <<< ADDED DURATION
                  )
                )
