...
BEGIN
    ...
    SELECT * INTO v_current_active_subscription
    FROM public.tenant_subscriptions
    WHERE tenant_id = p_tenant_id
      AND NOW() >= start_date
      AND (end_date IS NULL OR NOW() <= end_date)
    ORDER BY start_date DESC -- In case of overlaps, pick the one that started most recently
    LIMIT 1;
    ...
    IF v_current_active_subscription IS NOT NULL THEN
        ...
        RETURN QUERY SELECT 'activo'::TEXT, v_current_active_subscription.end_date, v_plan_name;
        RETURN;
    END IF;
    ...
    SELECT
        CASE
            ...
            ELSE 'cancelado'::TEXT
        END AS status,
        v_latest_subscription.end_date,
        v_plan_name;
END;
$$;
