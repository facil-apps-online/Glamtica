...
-- MIGRATION: Fix volatility of instrumented check_user_availability function
...
LANGUAGE plpgsql
VOLATILE -- CORRECTED from STABLE to VOLATILE
SECURITY DEFINER
AS $$
DECLARE
...
BEGIN
    RAISE NOTICE '[check_availability_DEBUG] START';
    RAISE NOTICE '[check_availability_DEBUG] Params: item_id=%, item_type=%, branch_id=%, tenant_id=%, app_date=%, app_time=%, duration=%', p_item_id, p_item_type, p_branch_id, p_tenant_id, p_appointment_date, p_appointment_time, p_duration_minutes;

    v_day_of_week := EXTRACT(DOW FROM p_appointment_date);
    RAISE NOTICE '[check_availability_DEBUG] Day of week: %', v_day_of_week;
    ...
    RAISE NOTICE '[check_availability_DEBUG] Tenant timezone from DB: %', v_tenant_timezone;

    IF v_tenant_timezone IS NULL THEN
        v_tenant_timezone := 'UTC';
        RAISE NOTICE '[check_availability_DEBUG] Tenant timezone is NULL, falling back to UTC.';
    END IF;

    -- 2. Convertir la hora de la cita a UTC
    ...
    RAISE NOTICE '[check_availability_DEBUG] Appointment Start UTC: %', v_appointment_start_utc;
    RAISE NOTICE '[check_availability_DEBUG] Appointment End UTC: %', v_appointment_end_utc;
    ...
    RAISE NOTICE '[check_availability_DEBUG] Appointment Range (for overlap checks): %', v_appointment_range;

    -- Usar una tabla temporal para almacenar los resultados y contarlos
    CREATE TEMP TABLE temp_available_users ON COMMIT DROP AS
            ...
            ( 
                EXISTS (
                    ...
                    WHERE us.user_id = tu.user_id
                      ...
                      AND TSRANGE(
                            ( (p_appointment_date::TEXT || ' ' || us.start_time::TEXT)::TIMESTAMP AT TIME ZONE COALESCE(au.raw_user_meta_data->>'timezone', v_tenant_timezone) AT TIME ZONE 'UTC' )::TIMESTAMP WITHOUT TIME ZONE,
                            ( (p_appointment_date::TEXT || ' ' || us.end_time::TEXT)::TIMESTAMP AT TIME ZONE COALESCE(au.raw_user_meta_data->>'timezone', v_tenant_timezone) AT TIME ZONE 'UTC' )::TIMESTAMP WITHOUT TIME ZONE
                          ) @> v_appointment_range
                )
                AND NOT EXISTS (
                    SELECT 1
                    ...
                    WHERE uto.user_id = tu.user_id
                      ...
                      AND (
                          (NOT uto.is_partial_day AND v_appointment_start_utc::DATE BETWEEN (uto.start_date AT TIME ZONE 'UTC')::DATE AND (uto.end_date AT TIME ZONE 'UTC')::DATE)
                          OR
                          (uto.is_partial_day AND v_appointment_range && TSRANGE(
                              (uto.start_date AT TIME ZONE 'UTC')::TIMESTAMP WITHOUT TIME ZONE,
                              (uto.end_date AT TIME ZONE 'UTC')::TIMESTAMP WITHOUT TIME ZONE
                          ))
                      )
                )
            )
    ...
    RAISE NOTICE '[check_availability_DEBUG] Found % available users matching all criteria.', v_user_count;
    RAISE NOTICE '[check_availability_DEBUG] END';

    RETURN QUERY SELECT * FROM temp_available_users;
END;
$$;
