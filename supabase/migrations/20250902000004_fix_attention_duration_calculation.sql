-- Migration to fix get_attentions_with_details RPC to calculate total duration correctly.
...
RETURNS TABLE(
    ...
    status text,
    notes text,
    total_duration_minutes integer, -- Added field
    clients json,
    ...
    attention_durations AS (
        ...
        FROM (
            SELECT
                attention_id,
                is_parallel,
                SUM(CASE WHEN NOT is_parallel THEN duration_minutes ELSE 0 END) OVER (PARTITION BY attention_id ORDER BY service_order, id) as sequential_timeline,
                LAG(SUM(CASE WHEN NOT is_parallel THEN duration_minutes ELSE 0 END) OVER (PARTITION BY attention_id ORDER BY service_order, id), 1, 0) OVER (PARTITION BY attention_id ORDER BY service_order, id) as start_offset,
                LAG(SUM(CASE WHEN NOT is_parallel THEN duration_minutes ELSE 0 END) OVER (PARTITION BY attention_id ORDER BY service_order, id), 1, 0) OVER (PARTITION BY attention_id ORDER BY service_order, id) + duration_minutes as end_offset
            FROM (
                SELECT 
                    s.attention_id,
                    s.is_parallel,
