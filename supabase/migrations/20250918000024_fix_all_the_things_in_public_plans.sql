...
BEGIN
    ...
    current_tariffs AS (
        SELECT DISTINCT ON (pt.subscription_plan_id)
            ...
            (SELECT pal.extra_unit_price FROM public.plan_asset_limits pal JOIN assets a ON pal.asset_id = a.id WHERE pal.plan_id = pt.subscription_plan_id AND a.asset_key = 'suc_glam' LIMIT 1) AS extra_branch_price,
            ...
            (SELECT (pal.bonus_on_extra->>'quantity')::INT FROM public.plan_asset_limits pal JOIN assets a ON pal.asset_id = a.id WHERE pal.plan_id = pt.subscription_plan_id AND a.asset_key = 'suc_glam' LIMIT 1) AS extra_branch_bonus_einvoices
