-- MIGRATION TO SIMPLIFY USER AVAILABILITY CHECK

-- Step 1: Drop the existing function.
DROP FUNCTION IF EXISTS public.check_user_availability(UUID, TEXT, UUID, UUID, TIMESTAMPTZ, INTEGER, UUID, UUID);

-- Step 2: Recreate the function WITHOUT the conflicting attentions check.
...
BEGIN
    ...
    SELECT DISTINCT ON (tu.user_id)
        tu.user_id,
        COLESCENCE(suc.commission_rate, 0.00) AS commission_rate,
