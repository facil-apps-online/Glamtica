-- Migration: Fix check_user_availability to use correct time_off columns
-- This corrects the column names from start_datetime/end_datetime to start_date/end_date.

DROP FUNCTION IF EXISTS public.check_user_availability(UUID, TEXT, UUID, UUID, DATE, TIME WITHOUT TIME ZONE, INTEGER, UUID) CASCADE;
    ...
    WHERE
        ...
        AND NOT EXISTS (
            ...
            WHERE uto.user_id = pu.user_id
              ...
              AND TSTZRANGE(uto.start_date, uto.end_date) && TSTZRANGE(v_appointment_start_utc, v_appointment_end_utc)
        )
