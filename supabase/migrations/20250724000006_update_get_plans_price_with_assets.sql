-- MIGRATION: Update get_subscription_plans_for_tenant to calculate price based on active assets.
...
BEGIN
    ...
    -- Call the main pricing function and adjust the price based on active assets
    RETURN QUERY
    SELECT
        gcp.plan_id,
        gcp.plan_name,
...
COMMENT ON FUNCTION public.get_subscription_plans_for_tenant(UUID) IS 'Fetches available subscription plans and calculates their final prices for a specific tenant, including the cost of their active branch assets.';
