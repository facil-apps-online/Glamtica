-- Migration: 20250802000002_consolidate_tenant_creation.sql
-- Description: Consolidates all previous tenant creation logic into a single, robust function
-- and removes all obsolete, legacy functions.

BEGIN;

-- Step 1: Drop all obsolete and legacy functions to clean up the database.
DROP FUNCTION IF EXISTS public.register_new_tenant(text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, double precision, double precision, text, text);
DROP FUNCTION IF EXISTS public.create_tenant(text, uuid, text, uuid, text, text, text, text, text, text, text, text, text, text, text, text, text, text, double precision, double precision);
DROP FUNCTION IF EXISTS public.create_tenant_admin(uuid, text, text);
DROP FUNCTION IF EXISTS public.create_tenant_with_admin(text, uuid, text, uuid, text, text, text, text, text, text, text, text, text, text, text, text, text, text, double precision, double precision, text, uuid);
DROP FUNCTION IF EXISTS public.create_tenant_user(uuid, text, text, text, text, uuid, uuid);
DROP FUNCTION IF EXISTS public.create_tenant_and_admin_logic(text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, double precision, double precision, text, text);

-- Step 2: Create the new canonical function for setting up a tenant for a new user.
-- This function is designed to be called by an Edge Function after the user has been created in auth.users.
...
BEGIN
    ...
    VALUES (v_new_tenant_id, 'Principal', TRUE, v_tenant_data ->> 'physical_address_line1')
...
COMMENT ON FUNCTION public.setup_tenant_for_new_user(UUID) IS 'v1: Canonical function to set up a complete tenant environment for a newly created user. Reads metadata from auth.users to perform setup.';
