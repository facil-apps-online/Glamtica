-- Migration: Fix email data type mismatch in get_tenant_users function.
-- Description: The auth.users.email column is character varying(255), but the
-- get_tenant_users function was defined to return TEXT for the email column.
...
-- Step 2: Create the new, corrected get_tenant_users function.
...
BEGIN
    ...
    SELECT
        ...
        u.id AS user_id,
        u.email::text, -- FIX: Explicitly cast email to TEXT
