-- supabase/migrations/20250729000020_fix_rpc_get_platform_level_assignments.sql
...
BEGIN
    ...
    WITH user_platform_roles AS (
        ...
        WHERE
            ...
            -- FIX: Asegurarse de que platform_id no sea nulo antes de intentar unir.
            -- Esto previene un crash si un objeto de asignación está malformado.
...
COMMENT ON FUNCTION public.get_platform_level_assignments() IS 'v2: Hardened against malformed assignment objects. Obtains platform-level roles (investor, app_super_admin) for all users, consolidating info from app_metadata and investor_platform_shares.';
