...
DROP POLICY IF EXISTS "Enable all operations for attentions" ON public.attentions;
CREATE POLICY "Enable all operations for attentions" 
ON public.attentions FOR ALL USING (true);

DROP POLICY IF EXISTS "Enable all operations for attention_services" ON public.attention_services;
CREATE POLICY "Enable all operations for attention_services" 
ON public.attention_services FOR ALL USING (true);

DROP POLICY IF EXISTS "Enable all operations for service_sessions" ON public.service_sessions;
CREATE POLICY "Enable all operations for service_sessions" 
ON public.service_sessions FOR ALL USING (true);

DROP POLICY IF EXISTS "Enable all operations for service_evidence" ON public.service_evidence;
CREATE POLICY "Enable all operations for service_evidence" 
ON public.service_evidence FOR ALL USING (true);

DROP POLICY IF EXISTS "Enable all operations for attention_products" ON public.attention_products;
...
DROP TRIGGER IF EXISTS update_attentions_updated_at ON public.attentions;
CREATE TRIGGER update_attentions_updated_at
  BEFORE UPDATE ON public.attentions
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_attention_services_updated_at ON public.attention_services;
...
DROP TRIGGER IF EXISTS update_service_evidence_updated_at ON public.service_evidence;
CREATE TRIGGER update_service_evidence_updated_at
  BEFORE UPDATE ON public.service_evidence
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_attention_products_updated_at ON public.attention_products;
...
DROP TRIGGER IF EXISTS calculate_service_session_duration ON public.service_sessions;
...
CREATE INDEX IF NOT EXISTS idx_attentions_date ON public.attentions(attention_date);
CREATE INDEX IF NOT EXISTS idx_attention_services_attention_id ON public.attention_services(attention_id);
CREATE INDEX IF NOT EXISTS idx_attention_services_stylist_id ON public.attention_services(stylist_id);
CREATE INDEX IF NOT EXISTS idx_service_sessions_attention_service_id ON public.service_sessions(attention_service_id);
CREATE INDEX IF NOT EXISTS idx_service_evidence_session_id ON public.service_evidence(service_session_id);

-- Migrar datos existentes de appointments a attentions
ALTER TABLE public.attentions DISABLE TRIGGER audit_attentions_changes;
ALTER TABLE public.attention_services DISABLE TRIGGER audit_attention_services_changes;
ALTER TABLE public.service_sessions DISABLE TRIGGER audit_service_sessions_changes;
ALTER TABLE public.service_evidence DISABLE TRIGGER audit_service_evidence_changes;
ALTER TABLE public.attention_products DISABLE TRIGGER audit_attention_products_changes;

INSERT INTO public.attentions (id, client_id, attention_date, attention_time, status, notes, total_amount, created_at, updated_at, tenant_id, branch_id)
SELECT id, client_id, appointment_date, appointment_time, status, notes, total_price, created_at, updated_at, tenant_id, branch_id
FROM public.appointments;

-- Migrar el servicio principal de cada appointment a attention_services
INSERT INTO public.attention_services (attention_id, service_id, stylist_id, service_price, service_order, status, created_at, updated_at, tenant_id, branch_id)
SELECT a.id, a.service_id, a.stylist_id, s.price, 1, 
  CASE 
    ...
    ELSE 'Pendiente'
  END,
  a.created_at, a.updated_at, a.tenant_id, a.branch_id
...
SELECT aes.appointment_id, aes.service_id, aes.stylist_id, aes.price, 
  ...
  'Pendiente',
  aes.created_at, aes.updated_at, aes.tenant_id, aes.branch_id
FROM public.appointment_extra_services aes;

-- Migrar sesiones principales a service_sessions
INSERT INTO public.service_sessions (attention_service_id, started_at, ended_at, duration_minutes, notes, created_at, updated_at, tenant_id, branch_id)
SELECT ats.id, aps.started_at, aps.ended_at, aps.duration_minutes, aps.notes, aps.created_at, aps.updated_at, aps.tenant_id, aps.branch_id
...
INSERT INTO public.service_sessions (attention_service_id, started_at, ended_at, duration_minutes, notes, created_at, updated_at, tenant_id, branch_id)
SELECT ats.id, ess.started_at, ess.ended_at, ess.duration_minutes, ess.notes, ess.created_at, ess.updated_at, ess.tenant_id, ess.branch_id
...
INSERT INTO public.attention_products (attention_id, product_id, quantity, unit_price, total_price, created_at, updated_at, tenant_id, branch_id)
SELECT appointment_id, product_id, quantity, unit_price, total_price, created_at, updated_at, tenant_id, branch_id
FROM public.appointment_products;

-- Migrar evidencia a service_evidence (esto requiere mapear las sesiones correctamente)
INSERT INTO public.service_evidence (service_session_id, attention_id, file_path, file_name, file_size, mime_type, uploaded_by, created_at, updated_at, tenant_id, branch_id)
SELECT ss.id, ss_ats.attention_id, ae.file_path, ae.file_name, ae.file_size, ae.mime_type, ae.uploaded_by, ae.created_at, ae.updated_at, ae.tenant_id, ae.branch_id
...
ALTER TABLE public.attentions ENABLE TRIGGER audit_attentions_changes;
ALTER TABLE public.attention_services ENABLE TRIGGER audit_attention_services_changes;
ALTER TABLE public.service_sessions ENABLE TRIGGER audit_service_sessions_changes;
ALTER TABLE public.service_evidence ENABLE TRIGGER audit_service_evidence_changes;
ALTER TABLE public.attention_products ENABLE TRIGGER audit_attention_products_changes;