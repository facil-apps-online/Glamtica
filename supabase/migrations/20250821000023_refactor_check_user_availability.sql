-- Migration: 20250821000023_refactor_check_user_availability.sql
-- Description: Refactors the check_user_availability function to use the get_tenant_users function
-- as the source of users. This makes the function more robust and consistent with the rest of the application.
...
BEGIN
            ...
            CASE
                WHEN p_item_type = 'service' THEN
                    (SELECT suc.commission_rate FROM public.service_user_commissions suc
                     WHERE suc.user_id = tu.user_id AND suc.service_id = p_item_id AND suc.branch_id = p_branch_id LIMIT 1)
        ...
        WHERE tu.branch_id = p_branch_id
          ...
          AND (
            (p_item_type = 'service' AND EXISTS (
                ...
                WHERE suc.user_id = tu.user_id AND suc.service_id = p_item_id AND suc.branch_id = p_branch_id
            ))
            OR
    ...
    LEFT JOIN public.user_schedules us ON us.user_id = cu.user_id AND us.day_of_week = v_day_of_week AND us.is_active = TRUE
    LEFT JOIN public.user_time_off uto ON uto.user_id = cu.user_id AND uto.status = 'approved'
    ...
    WHERE
        us.start_time IS NOT NULL AND us.end_time IS NOT NULL
        AND p_appointment_time >= us.start_time
        AND v_appointment_end_time <= us.end_time
        AND uto.user_id IS NULL;
END;
$$;
