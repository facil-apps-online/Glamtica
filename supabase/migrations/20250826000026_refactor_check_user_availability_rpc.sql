-- Migration: Refactor check_user_availability to use a single timestamptz parameter
...
-- Step 2: Create the new function with the updated signature
...
BEGIN
    ...
    v_appointment_range := TSRANGE(p_appointment_datetime, v_appointment_end_utc, '[]');
            ...
            ( -- Availability logic
                ...
                AND NOT EXISTS (
                    ...
                    WHERE aserv.user_id = tu.user_id
                      ...
                      AND TSRANGE(a.attention_datetime, a.attention_datetime + (aserv.duration_minutes || ' minutes')::interval, '[]') && v_appointment_range
                )
                -- Check for time off
                AND NOT EXISTS (
                    SELECT 1
                    ...
                    WHERE uto.user_id = tu.user_id
                      ...
                      AND (
                          ...
                          (uto.is_partial_day AND TSRANGE(p_appointment_datetime, v_appointment_end_utc, '[]') && TSRANGE(uto.start_date::date + uto.start_time, uto.end_date::date + uto.end_time, '[]'))
                      )
                )
            )
