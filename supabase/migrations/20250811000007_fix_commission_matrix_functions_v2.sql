-- Migración para corregir las funciones de matriz de comisiones de productos y servicios (v2).
...
BEGIN
  ...
  relevant_users AS (
    ...
    SELECT DISTINCT
        u.id as user_id,
        (u.raw_user_meta_data->>'first_name') || ' ' || (u.raw_user_meta_data->>'last_name') as user_name -- CAST implícito a text
    FROM
        auth.users u,
        jsonb_array_elements(u.raw_user_meta_data::jsonb->'assignments') AS assignment -- CAST explícito a jsonb
    WHERE
