...
BEGIN
    ...
    WITH tenant_users AS (
        SELECT tu.user_id, (tu.user_data->>'raw_user_meta_data')::jsonb->>'first_name' || ' ' || (tu.user_data->>'raw_user_meta_data')::jsonb->>'last_name' as user_name, b.id as branch_id, b.name as branch_name
        FROM get_tenant_users(p_tenant_id) tu
        LEFT JOIN branches b on b.id = (SELECT value->>'branch_id' FROM jsonb_array_elements(tu.user_data->'assignments') WHERE value->>'tenant_id' = p_tenant_id::text LIMIT 1)::uuid
    )
    SELECT
        uto.id,
        uto.user_id,
