-- supabase/migrations/20250711210000_fix_login_user_missing_apikey_param.sql

-- Drop the previous versions to ensure a clean slate
...
-- Recreate the function with the missing p_supabase_anon_key parameter
CREATE OR REPLACE FUNCTION public.login_user(
    ...
    p_supabase_anon_key TEXT DEFAULT NULL -- The missing parameter
)
RETURNS JSON AS $$
DECLARE
    user_record RECORD;
...
BEGIN
    ...
    -- Call Edge Function to generate JWT, now with the correct apikey parameter
    SELECT * FROM http_post(
        edge_function_url,
        payload_to_edge_function,
        'application/json',
        ARRAY[http_header('apikey', p_supabase_anon_key)] -- This will now work
