...
BEGIN
    ...
    IF p_branch_id IS NOT NULL THEN
        -- Assuming branches table has a 'code' column
...
COMMENT ON FUNCTION public.get_next_document_number(uuid, text, uuid, jsonb) IS 'Safely retrieves the next formatted document number for a given type using a format template, handling concurrency. V2 with template support.';
