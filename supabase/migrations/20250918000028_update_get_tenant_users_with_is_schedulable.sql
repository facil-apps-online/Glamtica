-- MIGRATION: Update get_tenant_users to include is_schedulable

-- Step 1: Drop the old function to be safe.
DROP FUNCTION IF EXISTS public.get_tenant_users(uuid);

-- Step 2: Create the function with the new is_schedulable field.
...
RETURNS TABLE (
  ...
  branch_name TEXT,
  status TEXT,
  is_schedulable BOOLEAN, -- Added is_schedulable
  avatar_url TEXT,
  base_salary NUMERIC,
