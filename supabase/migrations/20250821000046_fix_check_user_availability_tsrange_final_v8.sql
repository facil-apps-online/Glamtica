-- Migration: 20250821000046_fix_check_user_availability_tsrange_final_v8.sql
-- Description: Final fix for tsrange function call in check_user_availability by explicitly casting TIMESTAMPTZ to TIMESTAMP WITHOUT TIME ZONE for tsrange arguments.
...
DECLARE
    ...
    v_appointment_range TSRANGE; -- Rango de la cita
BEGIN
    ...
    -- Crear el rango de la cita, casteando a TIMESTAMP WITHOUT TIME ZONE
    ...
    WHERE
        ...
        AND EXISTS (
            ...
            WHERE us.user_id = tu.user_id
              ...
              -- Crear el rango del horario del usuario en UTC, casteando a TIMESTAMP WITHOUT TIME ZONE
              AND TSRANGE(
                  ...
                  ) @> v_appointment_range -- El horario del usuario debe contener el rango completo de la cita
        )
        AND NOT EXISTS (
            SELECT 1
            ...
            WHERE uto.user_id = tu.user_id
              ...
              AND (
                  ...
                  -- Aquí también necesitamos castear a TIMESTAMP WITHOUT TIME ZONE para TSRANGE
                  ...
                  (uto.is_partial_day AND v_appointment_start_utc::DATE = uto.start_date::DATE AND
                   TSRANGE(v_appointment_start_utc::TIMESTAMP WITHOUT TIME ZONE, v_appointment_end_utc::TIMESTAMP WITHOUT TIME ZONE) && TSRANGE(uto.start_date::TIMESTAMP WITHOUT TIME ZONE, uto.end_date::TIMESTAMP WITHOUT TIME ZONE))
              )
        );
END;
$$;