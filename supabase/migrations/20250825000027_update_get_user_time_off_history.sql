-- Migration: Update get_user_time_off_history to explicitly select columns from get_tenant_users

-- Step 1: Drop the old function.
DROP FUNCTION IF EXISTS get_user_time_off_history(uuid, uuid, text, text, date, date, uuid, text) CASCADE;

-- Step 2: Create the new, updated get_user_time_off_history function.
...
BEGIN
    IF p_search_term IS NOT NULL THEN
        search_words := string_to_array(p_search_term, ' ');
    END IF;

    RETURN QUERY
    WITH tenant_users AS (
        SELECT user_id, first_name, last_name, email, branch_id, branch_name FROM get_tenant_users(p_tenant_id)
    )
    ...
    JOIN
        tenant_users tu ON uto.user_id = tu.user_id AND uto.branch_id = tu.branch_id
    WHERE
        ...
        AND (
            ...
            (
                SELECT bool_and(
                    tu.first_name ILIKE '%' || sw.word || '%' OR
                    tu.last_name ILIKE '%' || sw.word || '%' OR
                    tu.email ILIKE '%' || sw.word || '%'
                ) FROM unnest(search_words) as sw(word)
            )
        );
END;
$$ LANGUAGE plpgsql;
