-- MIGRATION TO FIX AND SIMPLIFY USER AVAILABILITY CHECK

-- Step 1: Drop the existing function with the typo and unused parameter.
DROP FUNCTION IF EXISTS public.check_user_availability(UUID, TEXT, UUID, UUID, TIMESTAMPTZ, INTEGER, UUID, UUID);

-- Step 2: Recreate the function with the corrected COALESCE and simplified signature.
