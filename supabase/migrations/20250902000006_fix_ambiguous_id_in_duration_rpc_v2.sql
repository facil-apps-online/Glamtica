-- Migration to fix ambiguous id reference in get_attentions_with_details RPC (v2).

DROP FUNCTION IF EXISTS public.get_attents_with_details(uuid, uuid, uuid, text, timestamptz, timestamptz);
...
BEGIN
    ...
    attention_durations AS (
        ...
        FROM (
            SELECT
                attention_id,
                is_parallel,
                SUM(CASE WHEN NOT is_parallel THEN duration_minutes ELSE 0 END) OVER (PARTITION BY attention_id ORDER BY service_order, attention_service_id) as sequential_timeline,
                LAG(SUM(CASE WHEN NOT is_parallel THEN duration_minutes ELSE 0 END) OVER (PARTITION BY attention_id ORDER BY service_order, attention_service_id), 1, 0) OVER (PARTITION BY attention_id ORDER BY service_order, attention_service_id) as start_offset,
                LAG(SUM(CASE WHEN NOT is_parallel THEN duration_minutes ELSE 0 END) OVER (PARTITION BY attention_id ORDER BY service_order, attention_service_id), 1, 0) OVER (PARTITION BY attention_id ORDER BY service_order, attention_service_id) + duration_minutes as end_offset
            FROM (
                SELECT 
                    s.attention_id,
                    s.is_parallel,
                    ...
                    s.service_order,
                    s.id as attention_service_id -- Renamed for clarity
    ...
    LEFT JOIN attention_durations ad ON a.id = ad.attention_id
