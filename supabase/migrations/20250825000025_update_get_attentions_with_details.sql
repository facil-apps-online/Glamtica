-- Migration: Update get_attentions_with_details to explicitly select columns from get_tenant_users

-- Step 1: Drop the old function.
DROP FUNCTION IF EXISTS get_attentions_with_details(uuid, uuid, uuid, text, date) CASCADE;

-- Step 2: Create the new, updated get_attentions_with_details function.
      ...
      SELECT json_agg(
        json_build_object(
          ...
          'users', (
            ...
            FROM (SELECT user_id, first_name, last_name FROM get_tenant_users(p_tenant_id)) gtu
