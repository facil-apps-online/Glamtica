-- Migración para corregir la ambigüedad de la columna 'name' en la función create_tenant_with_admin

-- Reemplazamos la función existente con la versión corregida.
-- El único cambio es especificar 'roles.name' en la cláusula WHERE para eliminar la ambigüedad.
...
BEGIN
    ...
    -- 2. Obtener el ID del rol 'Tenant Superadmin' (CORREGIDO: especificando roles.name)
    SELECT id INTO tenant_admin_role_id FROM roles WHERE roles.name = 'Tenant Superadmin' AND tenant_id IS NULL LIMIT 1;
