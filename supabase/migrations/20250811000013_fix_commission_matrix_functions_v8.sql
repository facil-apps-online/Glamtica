-- Migración para corregir las funciones de matriz de comisiones de productos y servicios (v8).
...
BEGIN
  ...
  relevant_users AS (
    ...
    SELECT DISTINCT
        u.id as user_id,
        u.raw_user_meta_data->>'first_name' || ' ' || u.raw_user_meta_data->>'last_name' as user_name -- Sin CAST explícito
    FROM
        auth.users u,
        jsonb_array_elements(u.raw_app_meta_data->'assignments') AS assignment -- Sin CAST explícito
    WHERE
