...
const formSchema = z.object({
  ...
  platform_name: z.string().optional(), // Read-only field
...
export default function EditTenant() {
  ...
  useEffect(() => {
    if (tenant) {
      form.reset({
        ...tenant,
        platform_name: tenant.platform?.name || 'N/A',
        name: tenant.name || '',
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    const { platform_name, ...updateValues } = values; // Exclude platform_name from submission
    updateTenantMutation.mutate(
      { id: tenantId!, ...updateValues },
      {
        onSuccess: () => {
          toast({ title: 'Tenant Actualizado', description: 'El tenant ha sido actualizado exitosamente.' });
          navigate('/superadmin/tenants');
        },
        onError: (error) => {
          toast({ title: 'Error', description: `Error al actualizar: ${error.message}`, variant: 'destructive' });
        },
      }
    );
  };
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">Editar Tenant: {tenant?.name}</h1>
      </div>
      <Form {...form}>
          ...
          <div className="p-6 border rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <FormField control={form.control} name="platform_name" render={({ field }) => (
                <FormItem><FormLabel>Plataforma</FormLabel><FormControl><Input {...field} disabled readOnly /></FormControl><FormMessage /></FormItem>
              )} />
