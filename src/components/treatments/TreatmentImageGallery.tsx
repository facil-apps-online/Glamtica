import { useTreatmentImages, useAssociateTreatmentImage, useDeleteTreatmentImage, useSetPrimaryTreatmentImage } from "@/hooks/useTreatmentImages";
...
export const TreatmentImageGallery = ({ treatmentId }: TreatmentImageGalleryProps) => {
  const { data: images, isLoading, isError, refetch: refetchImages } = useTreatmentImages(treatmentId);
  const { mutate: deleteImage, isDeleting } = useDeleteTreatmentImage();
  const { mutate: setPrimaryImage } = useSetPrimaryTreatmentImage();
  const { mutate: associateImage, isPending: isAssociating } = useAssociateTreatmentImage();
  ...
  const handleDeleteClick = (imageToDeleteId: string, treatmentId: string, google_drive_file_id: string) => {
    setDeletingImageId(imageToDeleteId);
    deleteImage(
      { imageId: imageToDeleteId, treatmentId, google_drive_file_id },
      {
        onSuccess: () => {
      ...
      <div className="space-y-6">
        <div className="space-y-4 p-4 border rounded-lg">
          <FileUploader
            ...
            path_components={['tenant', tenantId, 'Treatment', treatmentId].filter(Boolean) as string[]}
            imageType="catalogo"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {images.map((image) => (
                <Card key={image.id} className="relative group overflow-hidden">
                  ...
                  >
                    <TreatmentImage imageUrl={image.google_drive_file_id} altText={`Imagen de tratamiento ${image.id}`} className="object-cover w-full h-32" />
                    {image.is_primary && (
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-300 flex items-center justify-center gap-2">
                      {!image.is_primary && deletingImageId !== image.id && (
                        <Button
                          ...
                          onClick={(e) => { e.stopPropagation(); setPrimaryImage({ treatmentId, imageId: image.id }); }}
                        >
                          <Star className="w-4 h-4" />
                        </Button>
                      )}
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction 
                                onClick={() => {
                                  ...
                                  handleDeleteClick(image.id, treatmentId, image.google_drive_file_id);
                                }}
                                disabled={deletingImageId === image.id}
                              >
